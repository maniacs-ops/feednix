...
void CursesProvider::eventHandler()
{
    ...
    while ((ch = getch()) != KEY_F(1) && ch != 'q')
    {
        ...
        switch (ch)
        {
            ...
            case 'r':{
                         if (postsMenu not_eq NULL && curItem not_eq NULL && postsItems not_eq NULL)
                         {
            ...
            case 'o':
                     ...
                     break;
            case 'O':{
                         std::string curItemUrl = feedly.getSinglePostData(item_index(curItem))->originURL;
            ...
            case 'A':{
                         ...
                         std::string streamId = feedly.getStreamId(item_name(current_item(ctgMenu)));
                         feedly.markCategoriesRead(streamId,
                                 lastEntryRead);
...
void CursesProvider::createCategoriesMenu()
{
    ...
    {
        if (it->first.compare("All") not_eq 0 && it->first.compare("Saved") != 0)
        {
...
void CursesProvider::createPostsMenu()
{
    ...
    const std::vector<PostData> *posts = feedly.givePostsFromStream("All", currentRank);
...
void CursesProvider::changeSelectedItem(
        MENU* curMenu,
        int   req
        )
{
...
void CursesProvider::postsMenuCallback(
        ITEM* item,
        bool  preview
        )
{
...
void CursesProvider::markItemRead(ITEM* item)
{
    if (item_opts(item) && item != NULL)
    {
        ...
        if (item_name(current_item(ctgMenu)) != std::string("Saved"))
        {
            ...
            feedly.markPostWithAction("markAsRead", const_cast<std::vector<std::string>*>(temp));
            numUnread--;
            numRead++;
            update_statusline("", NULL, true);
        }
...
void CursesProvider::print_in_middle(
        WINDOW      *win,
        int         starty,
        int         startx,
        int         width,
...
void CursesProvider::print_in_center(
        WINDOW      *win,
        int         starty,
        int         startx,
        int         height,
...
void CursesProvider::update_statusline(
        const char* update,
        const char* post,
        bool        showCounter
        )
{
    ...
    int statusLineLength = statusLine[0].empty() ? 0 : (statusLine[0].length() + 1);
    int statusLinePos = COLS - statusLine[0].length() - statusLine[2].length() - 2;
    mvprintw(LINES - 2, statusLineLength,
            statusLine[1].substr(0,
                statusLinePos).c_str());
