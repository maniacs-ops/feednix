...
class FeedlyProvider {
   public:
    ...
    void curl_cleanup();
    bool markPostWithAction(const std::string& action,
                            const std::vector<std::string>* ids);
    bool markCategoriesRead(const std::string& id,
                            const std::string& lastReadEntryId);
    bool addSubscription(bool newCategory, const std::string& feed,
                         std::vector<std::string> categories,
                         const std::string& title = "");
    const std::vector<PostData>* givePostsFromStream(
        const std::string& category, bool whichRank = 0);
