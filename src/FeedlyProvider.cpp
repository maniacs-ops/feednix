...
FeedlyProvider::FeedlyProvider()
{
    ...
    std::ifstream tokenFile(configPath.c_str(),
                            std::ifstream::binary);
...
const std::map<std::string, std::string>* FeedlyProvider::getLabels()
{
    ...
    if (data.fail() || curl_res not_eq CURLE_OK || not parsingSuccesful || not root.isArray())
    {
        if (not isLogStreamOpen)
        {
            openLogStream();
        ...
        )
{
    ...
    // Retrival URI for All, Saved, Uncategorized are Different than user streams
    if (category == "All")
    {
        std::string streamId = curl_easy_escape(curl,
                                               (user_data.categories["All"]).c_str(),
                                                0);
        curl_retrive("streams/contents?ranked=" + rank +
                                      "&count=" + rtrv_count +
                                      "&unreadOnly=true&streamId=" + streamId);
    }
    else if (category == "Uncategorized")
    {
        std::string streamId = curl_easy_escape(curl,
                                               (user_data.categories["Uncategorized"]).c_str(),
                                                0);
        curl_retrive("streams/contents?ranked=" + rank +
                                     "&count=" + rtrv_count +
                                     "&unreadOnly=true&streamId=" + streamId);
    ...
    {
        std::string streamId = curl_easy_escape(curl,
                                               (user_data.categories["Saved"]).c_str(),
                                                0);
        curl_retrive("streams/contents?ranked=" + rank +
                                     "&count=" + rtrv_count +
                                     "&unreadOnly=true&streamId=" + streamId);
    ...
    {
        std::string streamId = curl_easy_escape(curl, user_data.categories[category].c_str(), 0);
        curl_retrive("streams/" + streamId + "/contents?unreadOnly=true&ranked=" + rank +
                                                                      "&count=" + rtrv_count);
    }
    ...
    for (unsigned int i = 0; i < root["items"].size(); i++)
    {
        feeds.push_back(
            PostData{
                root["items"][i]["summary"]["content"].asString(),
...
bool FeedlyProvider::markPostWithAction(
...
bool FeedlyProvider::markCategoriesRead(
        const std::string& id,
...
bool FeedlyProvider::addSubscription(
        bool                     newCategory,
        ...
        )
{
    ...
    curl_easy_setopt(curl, CURLOPT_URL, ("https://cloud.feedly.com/v3/" + uri).c_str());
    curl_easy_setopt(curl, CURLOPT_WRITEDATA, data_holder);
...
void FeedlyProvider::openLogStream()
{
    ...
    {
        const char* log_path = std::string(HOME_PATH + "/.config/feednix/log.txt").c_str();
